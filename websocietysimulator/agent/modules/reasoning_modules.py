...
class ReasoningTOT(ReasoningBase):
    def __call__(self, task_description: str, feedback: str = ""):
        ...
        # modified prompt for TOT since the original prompt may lead to exceed the token limit
        prompt = """Solve the task step by step, but keep your reasoning very concise (no more than 3 short sentences).
        At the end, output ONLY the final answer as a Python list of item_id strings, with no explanations or extra text.
        Here are some examples.
        {examples}
        Here is the task:
        {task_description}"""
