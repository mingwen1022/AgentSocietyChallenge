...
class SchemaFitterIO(SchemaFitterBase):
    ...
    def __init__(self, llm, interaction_tool: Optional[InteractionTool] = None):
        ...
        self.profile_builder = (
            ProfileBuilder(interaction_tool) if interaction_tool else None
        )
    ...
    def create_prompt(self, schema, items, profile_type="user"):
        ...
        schema_str = (
            json.dumps(schema, indent=2) if isinstance(schema, dict) else str(schema)
        )
        items_str = (
...
- For each field, return a very short phrase and strictly keep in no more than 50 English words.
- Do NOT copy long review sentences; summarize concisely.
- DO NOT return null unless information truly cannot be found.
- Do NOT include explicit sexual, violent, hateful, or offensive language in the output.
- If reviews contain such content, summarize it in neutral terms (e.g., "mature themes", "dark tone") without explicit wording.
- All keys and all string values MUST be valid JSON strings: use double quotes around them.
- Inside string values, do NOT use double quote characters. If the original text has quotes, paraphrase it without quotes.
- Do NOT use backticks, comments, ellipsis "..." or trailing commas in JSON.
- Include "{entity_id_field}" in output.
- Output ONLY a valid JSON array, no markdown, no explanations, and no extra text before or after the JSON.
...
Output JSON array:"""
    ...
    def __call__(self, schema, items, profile_type="user"):
        ...
        logger.error(
            f"  [SchemaFitter] FAILED to extract valid JSON array from LLM response!"
        )
        self.output = []
        return []
