...
class InfoOrchestrator:
    ...
    def __call__(
        ...
        user_profile = (
            self._generate_profile(
                planner_steps, user_id, self.USER_PROFILE, candidate_list
            )
            if user_id
            else {}
        )
        ...
        user_params = (
            [k for k in user_profile.keys() if k != "user_id"] if user_profile else None
        )
        ...
        if candidate_list and self._requires_profile(planner_steps, self.ITEM_PROFILE):
            item_profiles = self._generate_all_item_profiles(
                planner_steps, candidate_list, user_params
            )

        if self.memory:
            if user_profile:
                self._store_profile_in_memory(
                    user_profile, self.USER_PROFILE, planner_steps
                )
            for item_profile in item_profiles:
                self._store_profile_in_memory(
                    item_profile, self.ITEM_PROFILE, planner_steps
                )
    ...
    def _generate_profile(
        ...
        if (
            not self._requires_profile(planner_steps, profile_type)
            or not self.schema_fitter
            or not entity_id
        ):
            return {}
        ...
        if profile_type == self.USER_PROFILE:
            ...
            params = self.item_retriever.retrieve_parameters(
                entity_id, planner_steps, candidate_list
            )
        else:
            params = (
                UserParameterRetriever.DEFAULT_PARAMETERS
                if profile_type == self.USER_PROFILE
                else ItemParameterRetriever.DEFAULT_PARAMETERS
            )
    ...
    def _generate_all_item_profiles(
        ...
        if (
            self.max_candidates_to_profile
            and len(candidate_list) > self.max_candidates_to_profile
        ):
        ...
        for batch_start in range(0, len(candidates_to_profile), batch_size):
            ...
            logger.info(
                f"✓ Batch {batch_num}/{num_batches} (items {batch_start+1}-{batch_end}) using keys: {item_params}"
            )

            # Generate profiles for this batch
            for item_id in batch_ids:
                try:
                    profile = self._call_schema_fitter(
                        item_params, item_id, self.ITEM_PROFILE
                    )
                    if profile:
    ...
    def _call_schema_fitter(
        self, params: List[str], entity_id: Optional[str], profile_type: str
    ) -> Dict[str, Any]:
